...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch('/api/data?collection=Menu', {
...
.then(result => {
  console.log('API result:', result); // شوف الشكل الحقيقي
  const menuData = result[0]?.Menu?.[0] ?? result[0];
  setData(menuData);
  setLoading(false);
})
      .catch(err => {
...
const getOpeningHours = async (language = 'ar') => {
  try {
    ...
    // Safe navigation in case structure is unexpected
